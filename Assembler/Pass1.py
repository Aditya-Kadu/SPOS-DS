...
for line_tokens in source_line_tokens:
    ...
    ic_line = "{} ".format(
        location_cntr if mnemonic_class != "AD" else "   ") + ic_line
    ...
    if mnemonic_str == "ORIGIN":
        if "+" in operand1 or "-" in operand1:
            ...
            symbol_name, constant = operand1.replace(
                "+", " ").replace("-", " ").split()
            _, symbol_addr = find_symbol(symbol_name)
            # location_cntr = int(
            #     eval(operand1.replace(symbol_name, str(symbol_addr))))
            if "+" in operand1:
                location_cntr = symbol_addr+int(constant)
            if "-" in operand1:
                location_cntr = symbol_addr-int(constant)
            ic_line += "(C,{})".format(location_cntr)
        else:
    ...
    if mnemonics[mnemonic_str][0] == "IS":
        if mnemonic_str == "READ" or mnemonic_str == "PRINT":
            ...
            if "=" in operand2:
                ...
                ic_line += "({}) (L,{})".format(
                    register_codes[operand1], lit_tab_ptr)
                ...
                symbol_index = update_symbol(operand2, -1)
                ic_line += "({}) (S,{})".format(
                    register_codes[operand1], symbol_index)
        location_cntr += 1
...
#     file.write("\n".join(["{} {}".format(s, addr) for (s, addr) in symtab]))
